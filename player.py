        ...
        with open(prediction_saving_dir, 'r') as file:
            ...
            for line in file:
                ...
                if len(parts) == 5:
                    ...
                    frame_num_and_bounding_box_dict[frame_num] = (xmin, ymin, xmax, ymax)
                    for i in range(1,3):
                        frame_num_and_bounding_box_dict[frame_num+i] = (xmin, ymin, xmax, ymax)
                        frame_num_and_bounding_box_dict[frame_num-i] = (xmin, ymin, xmax, ymax)
    ...
    def run(self):
        while self.running:
            ...
            if not self.paused:
                ...
                if self.current_position in self.marked_frames:

                    print(self.predicted_results[self.current_position])
                    xmin, ymin, xmax, ymax = self.predicted_results[self.current_position]
